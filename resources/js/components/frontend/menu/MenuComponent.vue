    ...
    <section class="mb-16 mt-8">
        <div class="container">
            ...
            <div
                ...
                class="flex flex-wrap gap-3 w-full mb-12 veg-navs"
            >
                <button
                    :disabled="
                        itemProps.type !== null &&
                        itemProps.type === itemTypeEnum.VEG
                    "
                    @click.prevent="
                        itemProps.type === itemTypeEnum.NON_VEG
                            ? itemTypeReset()
                            : itemTypeSet(itemTypeEnum.NON_VEG)
                    "
                    :class="
                        itemProps.type === itemTypeEnum.NON_VEG
                            ? 'veg-active'
                ...
                <button
                    ...
                    @click.prevent="
                        itemProps.type === itemTypeEnum.VEG
                            ? itemTypeReset()
                            : itemTypeSet(itemTypeEnum.VEG)
                    "
                    :class="
                        itemProps.type === itemTypeEnum.VEG ? 'veg-active' : ''
                    "
                    type="button"
                    class="flex items-center gap-3 w-fit pl-3 pr-4 py-1.5 rounded-3xl transition hover:shadow-filter hover:bg-white bg-[#EFF0F6]"
                >
                    <img
                        :src="setting.image_non_vag"
                        alt="category"
                        class="h-6"
                    />
                    <span class="capitalize text-sm font-medium text-heading">{{
...
export default {
    ...
    mounted() {
        ...
        this.$store
            .dispatch("frontendItemCategory/lists", this.categoryProps.search)
            .then((res) => {
    ...
    methods: {
        ...
        categoryShow: function () {
            if (
                ...
                this.loading.isActive = true;
                this.$store
                    .dispatch("frontendItemCategory/show", {
