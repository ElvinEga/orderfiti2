    ...
    <div class="col-12">
        <div class="db-card">
            <div class="db-card-header border-none">
                <h3 class="db-card-title">Zones</h3>
                <div class="db-card-filter">
                    ...
                    <ZonesCreateComponent :props="props" />
                </div>
            </div>
            ...
            <div class="db-table-responsive">
                <table class="db-table stripe" id="print">
                    ...
                    <tbody class="db-table-body" v-if="zones.length > 0">
                        <tr class="db-table-body-tr" v-for="zones in zones" :key="zones">
                            <td class="db-table-body-td">{{ zones.name }}</td>
                            <td class="db-table-body-td">{{ zones.size }}</td>
                            <td class="db-table-body-td">
                                <span :class="statusClass(zones.status)">
                                    {{ enums.statusEnumArray[zones.status] }}
                                </span>
                            </td>
                            <td class="db-table-body-td hidden-print"
                                v-if="permissionChecker('dining_tables_show') || permissionChecker('dining_tables_edit') || permissionChecker('dining_tables_delete')">
                                <div class="flex justify-start items-center sm:items-start sm:justify-start gap-1.5">
                                    <SmIconQrCodeComponent :link="zones.qr" />
                                    <SmIconViewComponent :link="'admin.zones.show'" :id="zones.id"
                                        v-if="permissionChecker('dining_tables_show')" />
                                    <SmIconSidebarModalEditComponent @click="edit(zones)"
                                        v-if="permissionChecker('dining_tables_edit')" />
                                    <SmIconDeleteComponent @click="destroy(zones.id)"
                                        v-if="permissionChecker('dining_tables_delete') && demoChecker(zones.id)" />
                                </div>
                            </td>
                        </tr>
                    </tbody>
...
import ZonesCreateComponent from "./ZonesCreateComponent";
...
export default {
    ...
    computed: {
        ...
        pagination: function () {
            return this.$store.getters['zones/pagination'];
        },
        paginationPage: function () {
            return this.$store.getters['zones/page'];
        }
    },
    mounted() {
        this.list();
    ...
    methods: {
        ...
        list: function (page = 1) {
            ...
            this.$store.dispatch('zones/lists', this.props.search).then(res => {
        ...
        edit: function (zones) {
            ...
            this.loading.isActive = true;
            this.$store.dispatch('zones/edit', zones.id);
            this.props.form = {
        ...
        destroy: function (id) {
            appService.destroyConfirmation().then((res) => {
                try {
                    this.loading.isActive = true;
                    this.$store.dispatch('zones/destroy', { id: id, search: this.props.search }).then((res) => {
                        this.loading.isActive = false;
        ...
        xls: function () {
            this.loading.isActive = true;
            this.$store.dispatch("zones/export", this.props.search).then((res) => {
                this.loading.isActive = false;
